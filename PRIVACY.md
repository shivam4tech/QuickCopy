...
Pluk does not collect, transmit, or store any personal data. Everything the extension does — OCR, clipboard writes, and PDF viewing — happens locally on your device.

## Data collection

Pluk collects **no data**:

- **No personal information.** Pluk has no accounts, no logins, and no profile of any kind.
...
| `<all_urls>` (host permission) | Render the capture overlay and run OCR on any page — including videos, images, and built-in PDF viewers — no matter which site you are on. This permission is used only to draw the overlay and read the screen; Pluk never reads, modifies, or uploads page content. |
