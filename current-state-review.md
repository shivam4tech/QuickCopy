...
This review reflects the current codebase after the recent rounds of work: emoji support removed entirely, local-first OCR initialization (no CSP probe), dual-language support with IDB-traineddata management, `gzip: false` worker config, base64 traineddata transport, Trusted Types patch, the consumer-facing options page redesign (Recognition Mode, Downloaded Languages with storage accounting, Copying section with configurable panel dismiss), the liquid-glass dark/light/system theme system (v1.2.0), the custom (non-native) dropdown that keeps popup styling consistent across Chrome and Firefox, the configurable drag modifier (Alt/Option+Shift or Ctrl/Cmd+Drag), PDF page-number navigation, the Firefox AMO-clean build (0 errors, `data_collection_permissions: none`), and the PDF window's instant multi-capture (the overlay arms per drag instead of waiting for the side panel to close, so consecutive captures feel as fast as on regular pages).
